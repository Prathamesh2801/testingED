...
import { deleteUserDataClient, getUserDataByClient } from "../utils/ClientData";
...
export default function ClientDataTable({ onRefresh }) {
      ...
      if (response) {
        ...
        if (response.length > 0) {
          ...
          const filteredColumns = allKeys.filter(
            (key) => !["__v", "_id", "Created_AT"].includes(key)
          );
  ...
  async function handleDelete(userID) {
    toast.promise(
      deleteUserDataClient(userID).then(() => fetchClientData()),
      {
  ...
  function handleEdit(userID) {
    ...
    navigate(
      `/clientDashboard?view=create&userId=${encodeURIComponent(userID)}`
    );
  }
  ...
  const formatCellContent = (key, value) => {
    ...
    if (key === "QRPath" && value) {
      return (
        <button
          onClick={() => setSelectedQRCode(value)}
        ...
        <div className="px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            ...
            <div className="ml-auto mr-5 flex items-center space-x-4">
              {eventData?.Event_WhatsApp_Column_Name &&
                eventData?.Event_WhatsApp_Template_ID && (
                  <button className="flex items-center gap-2 p-2 pr-4 text-sm font-medium text-green-600  bg-transparent hover:bg-green-100 cursor-pointer border border-green-600 transition-colors duration-200 ease-in-out rounded-full hover:border-none ">
                    {" "}
              ...
              {eventData?.IsEmail === "1" && (
                <button className="flex items-center gap-2 p-2  pr-4  text-sm font-medium text-red-600  bg-transparent hover:bg-red-100 cursor-pointer border border-red-600 transition-colors duration-200 ease-in-out rounded-full hover:border-none ">

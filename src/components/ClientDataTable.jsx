...
export default function ClientDataTable() {
      ...
      try {
        ...
        if (response) {
          ...
          if (response.length > 0) {
            ...
            const filteredColumns = allKeys.filter(key => 
              !['__v', '_id','Created_AT'].includes(key)
            );
  ...
  const filteredData = useMemo(() => {
    return userData.filter(item => fuzzySearch(item, searchTerm));
  }, [userData, searchTerm]);
  ...
  const formatCellContent = (key, value) => {
    ...
    // Handle boolean values like Scanned
    if (typeof value === 'boolean' || value === '0' || value === '1') {
      const boolValue = value === true || value === '1';
      return (
        <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${boolValue ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
          {boolValue ? 'Yes' : 'No'}
        </span>
      );
    }
      ...
      <div className="fixed inset-0 bg-gray-600  bg-opacity-75 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
          ...
          <div className="flex justify-center">
            <img 
              src={qrPath.startsWith('http') ? qrPath : `${process.env.API_BASE_URL || ''}/${qrPath}`} 
              alt="QR Code" 
  ...
  return (
    <div className="bg-white shadow rounded-lg overflow-hidden">
      ...
      {/* QR Code Modal */}
      {selectedQRCode && (
        <QRCodeModal qrPath={selectedQRCode} onClose={() => setSelectedQRCode(null)} />
      )}
    </div>
  );
}
...
import { addFaceRegister } from "../utils/FaceRegister";
import ProgressFace from '../assets/img/progressFaceGif.gif'
...
export default function RegistrationForm() {
  ...
  useEffect(() => {
    const updateBg = () => {
      if (window.matchMedia("(min-width:1024px)").matches) {
        setBgUrl(
          desktopImgURL || tabletImgURL || mobileImgURL || fallbackGradient
        );
  ...
  return (
    <div
      className={`flex min-h-screen flex-col items-center pt-8 p-4 relative`}
      style={{
    ...
    >
      ...
      <div className="w-full max-w-sm flex flex-col items-center relative z-10">
        ...
        {currentScreen === "loading" && (
          <LoadingScreen onComplete={goToSuccess} eventData={eventData} />
        )}
        ...
        {currentScreen === "download" && (
          <DownloadScreen onComplete={goToRegister} />
        )}
        {currentScreen === "error" && (
          <ErrorScreen onHomeClick={goToRegister} errorMessage={error} />
        )}
      </div>
      ...
      >
        <div className="text-center">
          <div className="mb-6 text-gray-700">
            <p className="mb-4">
              This event requires facial recognition for registration.
            </p>
      ...
      >
        <div className="text-center">
          <div className="mb-6 text-gray-700">
            <p className="mb-4">
              Without facial recognition, some event features may be limited.
            </p>
...
function InputField({ field, value, onChange }) {
  ...
  // ✅ NEW: Extract max length from varchar(n)
  const extractLengthLimit = (typeString) => {
    const match = typeString.match(/varchar\((\d+)\)/i);
    return match ? parseInt(match[1], 10) : undefined;
  };

  const inputType = getInputType(field.type);
  const enumValues = getEnumValues(field.type);
  const maxLength = extractLengthLimit(field.type);

  return (
    <div className="mb-4">
      <label
        htmlFor={field.name}
        className="block mb-2 text-sm font-medium text-white tracking-wide uppercase"
      >
      ...
      {inputType === "file" ? (
        <div className="flex flex-col gap-1">
          <input
            ...
            accept={
              field.allowedTypes
                ? field.allowedTypes.map((type) => `.${type}`).join(",")
                : ""
            }
            name={field.name}
            id={field.name}
          ...
          <p className="text-xs text-gray-300 tracking-wider mb-1">
            {field.allowedTypes?.join(", ").toUpperCase()} (MAX.{" "}
  ...
  return (
    <>
      <div>
        <img
          alt="Company Logo"
          src={`${API_BASE_URL}/uploads/event_logos/${eventData.event.Event_Logo}`}
          className="h-20 w-20 my-1 border-2 border-white rounded-full"
        />
      </div>

      <h1 className="text-4xl text-white mb-1 font-bold tracking-wider">
        {eventData?.event?.Event_Name || "Event Name"}
      </h1>
      <p className="text-white text-2xl tracking-wider mb-6 font-bold">
        Create an account
      </p>
...
function FaceRecScreen({
  ...
  const [faceRecUUIDs, setFaceRecUUIDs] = useState([]);
  const [currentPhotoIndex, setCCurrentPhotoIndex] = useState(0);
  ...
  const generateFilename = () => {
    ...
    const month = (now.getMonth() + 1).toString().padStart(2, "0"); // e.g., "06"
    const day = now.getDate().toString().padStart(2, "0"); // e.g., "04"
  ...
  const capture = useCallback(async () => {
    ...
    try {
      ...
      // Call addFaceRegister API
      const response = await addFaceRegister({
        eventID: eventId,
        photo: file,
      });
      ...
      if (response.Status === true) {
        ...
        // Store the filename from response as UUID
        const newFaceRecUUIDs = [...faceRecUUIDs, response.Data.filename];
        setFaceRecUUIDs(newFaceRecUUIDs);

        // Store captured photo for display
        const newCapturedPhotos = [...capturedPhotos, imageSrc];
        setCapturedPhotos(newCapturedPhotos);

        // Show success toast
        toast.success(
          `Photo ${photoNumber}/3 captured successfully! Quality: ${response.Data.quality}`,
          {
            duration: 3000,
            position: "top-center",
          }
        ...
        // Check if we have captured all 3 photos
        if (photoNumber === 3) {
          // All 3 photos captured, proceed with registration
          await handleFinalRegistration(newFaceRecUUIDs);
        } else {
          // Move to next photo
          setCCurrentPhotoIndex(photoNumber);
        }
      } else {
        throw new Error(response.Message || "Failed to register face");
      }
    } catch (error) {
      console.error("Face capture error:", error.response.data.Message);
      toast.error(
        error.response.data.Message ||
  ...
  }, [currentPhotoIndex, capturedPhotos, faceRecUUIDs, eventId]);

  const handleFinalRegistration = async (uuids) => {
    try {
      setLoading(true);

      // Add the 3 face recognition UUIDs to form data
      const finalFormData = {
        ...formData,
        FaceRec_UUID1: uuids[0],
        FaceRec_UUID2: uuids[1],
        FaceRec_UUID3: uuids[2],
      };

      console.log("Final form data with face UUIDs:", finalFormData);

      // Call registration API with face recognition data
      const response = await registerEvent(eventId, finalFormData);
      console.log("Registration response:", response);
  ...
  const getInstructionText = () => {
    if (currentPhotoIndex === 0) {
      return "Please position your face in the camera and capture your first photo";
    } else if (currentPhotoIndex === 1) {
      return "Great! Now capture your second photo - try a slightly different angle";
    } else if (currentPhotoIndex === 2) {
      return "Perfect! One more photo needed - make sure your face is clearly visible";
    }
    return "";
  };
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-md">
      ...
      <div className="text-white text-center">
        <h2 className="text-2xl font-bold mb-2">Facial Recognition</h2>
        <p className="text-lg">Photo {currentPhotoIndex + 1} of 3</p>
        <p className="text-sm mt-2 px-4">{getInstructionText()}</p>
      </div>

      {/* Progress bar */}
      <div className="w-full bg-white/20 rounded-full h-2">
        <div
          className="bg-[#FFA60D] h-2 rounded-full transition-all duration-300"
          style={{ width: `${(currentPhotoIndex / 3) * 100}%` }}
        ></div>
      </div>
      ...
      {inProgress && (
        <div className="bg-white p-2 rounded-lg shadow-md">
          <img src={ProgressFace} alt="Progress Gif" />
        </div>
      )}

      {/* Previously captured photos preview */}
          ...
          {capturedPhotos.map((photo, index) => (
            <div key={index} className="relative">
              <img
                ...
                className="w-16 h-20 rounded border-2 border-green-400 object-cover"
              />
      ...
      <button
        ...
        `}
      >
        {loading ? "Processing..." : `Capture Photo ${currentPhotoIndex + 1}`}
      </button>

      {/* Help text */}
      <div className="text-white/80 text-center text-sm px-4">
        <p>Make sure your face is well-lit and clearly visible</p>
        <p>All 3 photos are required for registration</p>
      </div>
    </div>
  );
}
...
function DownloadScreen({ onComplete }) {
  ...
  return (
    <>
      ...
      <h2 className="text-white text-3xl tracking-wider font-bold text-center">
